...
def show_fio_warning(card_data):
    ...
    if current_template in target_templates or is_ekc_obrazec or is_tobolsk_obrazec:
        ...
        if current_template in ["ЭКЦ", "ЭКЦ_нет результата"] or is_ekc_obrazec or is_tobolsk_obrazec:
        ...
        elif current_template == "СВО_Молов БВП_образец_прямая идентификация":
            base_message = f"НОМЕР ЗАКЛЮЧЕНИЯ - {card_data.get('НОМ', 'N/A')}\n\nПрямая идентификация!"
            need_check_ind = False
        ...
        elif current_template in ["СВО_Ростов_образец_родственники", "СВО_Ростов_образец_родственники_нет результата_RT", "СВО_Ростов_образец_родственники_нет результата_форез"]:
            ...
            Панель 1 - {card_data.get('ПАН', 'N/A')}
            Панель 2 - {card_data.get('ПАН2', 'N/A')}
            Панель 3 - {card_data.get('ПАН3', 'N/A')}
            Панель 4 - {card_data.get('ПАН4', 'N/A')}"""
            need_check_ind = False  # ИНД не проверяем
            ...
            try:
                ...
                try:
                    ...
                    try:
                        ...
                        if column_1_value in ("СВО_Молов_образец_прямая идентификация", "СВО_Молов БВП_образец_прямая идентификация", "СВО_Ростов_образец_прямая идентификация"):
...
def create_and_fill_envelope(card_data):
    ...
    try:
        ...
        # Безопасное извлечение и приведение к строке значений ФИО и номера
        fi = str(card_data.get('ФИ', '')).strip() if card_data.get('ФИ') is not None else ''
        ii = str(card_data.get('ИИ', '')).strip() if card_data.get('ИИ') is not None else ''
        oi = str(card_data.get('ОИ', '')).strip() if card_data.get('ОИ') is not None else ''
        nom = str(card_data.get('НОМ', '')).strip() if card_data.get('НОМ') is not None else ''

        logging.debug(f"[create_and_fill_envelope] Исходные значения: fi='{fi}', ii='{ii}', oi='{oi}', nom='{nom}'")

        # Формируем список только непустых частей (исключая строку 'None')
        fio_parts = [part for part in [fi, ii, oi] if part and part != 'None']
        fio_string = ' '.join(fio_parts)

        logging.debug(f"[create_and_fill_envelope] Сформированная ФИО-строка: '{fio_string}'")

        if fio_string:
            output_filename = f"{fio_string} {nom}-26_конверт.docx"
        else:
            output_filename = f"{nom}-26_конверт.docx"
            logging.warning("[create_and_fill_envelope] Все части ФИО отсутствуют, имя файла конверта будет содержать только номер дела")

        logging.info(f"[create_and_fill_envelope] Имя файла конверта: {output_filename}")
    ...
    try:
        ...
        try:
            if is_ekc_type:
                ...
                # Шаг 2: Удаляем последний раздел полностью с сохранением форматирования
                logging.info("4. Удаление последнего раздела...")
                ...
                doc.Repaginate()
                last_page = doc.ComputeStatistics(2)
                logging.info(f"5. Количество страниц после удаления раздела: {last_page}")

                # Шаг 3: Изменение цвета шрифта колонтитулов первой страницы на черный
                logging.info("6. Изменение цвета шрифта колонтитулов первой страницы на черный...")
                try:
                ...
                # Шаг 4: Включаем зеркальные поля для печати всего документа
                logging.info("7. Включение зеркальных полей для печати всего документа...")
                ...
                # Шаг 5: Печатаем весь документ в одном экземпляре
                last_page = doc.ComputeStatistics(2)
                logging.info(f"8. Количество страниц перед печатью всего документа: {last_page}")
                logging.info("9. Печать всего документа в одном экземпляре...")
                doc.PrintOut(PrintToFile=False, Range=win32.constants.wdPrintRangeOfPages, Pages=f"1-{last_page}", Copies=1, Collate=True)

                logging.info("10. Упрощенная печать для типа ЭКЦ завершена.")

                # Шаг 6: Сохраняем изменённый документ в новом файле
                ...
                #logging.info(f"11. Сохранение изменённого документа в {modified_path}...")
                ...
                # Создаем новое имя файла с суффиксом "_modified"
                #modified_path = original_path.with_name(original_path.stem + "_modified" + original_path.suffix)
                #logging.info(f"13. Сохранение изменённого документа в {modified_path}...")
                #doc.SaveAs(str(modified_path))
...
def create_and_fill_document(card_data, txt_data_for_word, template_name):
    ...
    try:
        ...
        if card_data.get("1") in ["СВО_Молов_образец_родственники", "СВО_Молов_образец_прямая идентификация", "СВО_Молов БВП_образец_прямая идентификация", "СВО_Ростов_образец_родственники", "СВО_Ростов_образец_прямая идентификация"]:
            # Безопасное извлечение и преобразование значений
            fi = str(card_data.get('ФИ', '')).strip() if card_data.get('ФИ') is not None else ''
            ii = str(card_data.get('ИИ', '')).strip() if card_data.get('ИИ') is not None else ''
            oi = str(card_data.get('ОИ', '')).strip() if card_data.get('ОИ') is not None else ''
...
def move_to_final_location(card_data, word_file_path, excel_file_path=None):
    ...
    try:
        ...
        elif card_data.get("1") == "СВО_Молов БВП_образец_прямая идентификация":
            final_folder = Path(r"U:\ГЕНЕТИКА НОВОЕ ЗДАНИЕ\Ростов СВО\Личные профили 2-ДНК (прямая идентификация)")
...
@rebuild_win32com_cache_on_error
def get_document_page_count(doc_path: str) -> int:
    """
    Определяет количество страниц в документе Word с использованием win32com.
    При ошибке использует эвристический метод.
    """
    logging.info("=== Начало определения количества страниц ===")
    try:
        if not os.path.exists(doc_path):
            logging.error(f"Файл документа не найден: {doc_path}")
        ...
        try:
            ...
            doc.Close(SaveChanges=False)
            word.Quit()
            logging.info(f"Определено страниц через win32com: {page_count}")
            return page_count
        except Exception as e:
            logging.warning(f"Ошибка win32com: {e}. Переход на эвристику.")
            return estimate_page_count(doc_path)
        finally:
            try:
                pythoncom.CoUninitialize()
            except Exception:
        ...
        logging.error(f"Критическая ошибка при подсчете страниц: {e}")
        return 1

def estimate_page_count(doc_path: str, avg_lines_per_page: int = 35) -> int:
    """Эвристическая оценка количества страниц."""
    try:
        doc = Document(doc_path)
        total_paragraphs = len(doc.paragraphs)
        total_tables = len(doc.tables)
        total_elements = total_paragraphs + (total_tables * 7)
        estimated_pages = max(1, int(total_elements / avg_lines_per_page) + 1)
        corrected_pages = estimated_pages + 2
        logging.info(f"Эвристическая оценка: {corrected_pages} страниц")
        return corrected_pages
    except Exception as e:
        logging.error(f"Ошибка эвристической оценки: {e}")
        return 5

@rebuild_win32com_cache_on_error
def update_footer_with_page_count(doc_path: str) -> None:
    """
    Обновляет метку [КОЛ1] в колонтитулах на (количество страниц - 2).
    Использует win32com для гарантированного поиска текста в таблицах и связанных колонтитулах.
    """
    logging.info("=== Начало обновления колонтитулов (через win32com) ===")
    if not os.path.exists(doc_path):
        logging.error(f"Файл не найден: {doc_path}")
        return
    ...
    try:
        ...
        page_count = doc.ComputeStatistics(2)  # 2 = wdStatisticPages
        pages_to_insert = max(1, page_count - 2)
        logging.info(f"Всего страниц: {page_count}, вставляем: {pages_to_insert}")
        
        replacements_made = 0
        
        # Константы типов колонтитулов Word
        # 1 = wdHeaderFooterPrimary (Основной / нечетные)
        # 2 = wdHeaderFooterFirstPage (Первой страницы)
        # 3 = wdHeaderFooterEvenPages (Четные страницы)
        footer_types = [1, 2, 3]
        ...
        for section_idx in range(1, doc.Sections.Count + 1):
            ...
            for ft in footer_types:
                try:
                    footer = section.Footers(ft)
                    if footer.Exists:
                        # Проверяем наличие метки в тексте всего диапазона колонтитула
                        # Это видит текст внутри таблиц и скрытых элементов!
                        if "[КОЛ1]" in footer.Range.Text:
                            find = footer.Range.Find
                            find.ClearFormatting()
                            find.Replacement.ClearFormatting()
                            find.Text = "[КОЛ1]"
                            find.Replacement.Text = str(pages_to_insert)
                            # 2 = wdReplaceAll
                            if find.Execute(Replace=2):
                                replacements_made += 1
                                logging.info(f"Метка заменена в секции {section_idx}, тип колонтитула {ft}")
                except Exception as e:
                    logging.debug(f"Пропуск колонтитула {ft} в секции {section_idx}: {e}")
                    
        if replacements_made > 0:
            doc.Save()
            doc.Close(SaveChanges=True)
            doc_closed = True
            logging.info(f"Документ сохранен. Замен выполнено: {replacements_made}")
        else:
            logging.warning("Метка [КОЛ1] не найдена ни в одном колонтитуле (проверка через Word COM).")
            
    except Exception as e:
        logging.exception(f"Ошибка при обновлении колонтитулов через win32com: {e}")
    finally:
        if doc and not doc_closed:
            try: 
                doc.Close(SaveChanges=False)
            except Exception as close_e:
                logging.debug(f"Ошибка при закрытии документа: {close_e}")
        if word:
            try: 
                word.Quit()
            except Exception as quit_e:
                logging.debug(f"Ошибка при закрытии Word: {quit_e}")
        pythoncom.CoUninitialize()

def verify_footer_update(doc_path: str) -> bool:
    """Проверяет успешность замены метки [КОЛ1] через win32com."""
    logging.info("=== Начало проверки обновления колонтитулов (через win32com) ===")
    pythoncom.CoInitialize()
    word = None
    doc = None
    doc_closed = False
            ...
            for ft in footer_types:
                try:
                    ...
                    if footer.Exists:
                        ...
                        if "[КОЛ1]" in text:
                            markers_found += 1
                        if str(pages_to_insert) in text:
                            replacements_found += 1
                except Exception:
                    pass
                    
        if markers_found == 0 and replacements_found > 0:
            logging.info(f"Проверка успешна: найдено {replacements_found} колонтитулов с числом {pages_to_insert}")
            return True
        elif markers_found > 0:
            logging.warning(f"Обнаружено {markers_found} незамененных меток [КОЛ1]")
            return False
        else:
            logging.info("Метки и замены не найдены (возможно, шаблон не содержит метки).")
            return True
            
    except Exception as e:
        logging.exception(f"Ошибка при проверке колонтитулов: {e}")
        return False
    finally:
        if doc and not doc_closed:
            try: 
...
def archive_and_print_document(source_path, archive_folder, ask_before_print=True, card_data=None):
    ...
    try:
        ...
        # Обновление колонтитула с количеством страниц
        try:
            logging.info("=== Начало обновления колонтитула с количеством страниц ===")
            update_footer_with_page_count(str(source_path))
            if verify_footer_update(str(source_path)):
                logging.info("Проверка обновления колонтитула прошла успешно")
            else:
                logging.warning("Проверка обновления колонтитула не прошла успешно.")
        except Exception as e:
            logging.error(f"Ошибка при обновлении колонтитула (не прерываем выполнение): {e}")
        ...
        if card_data is not None and card_data.get("1") in [
    ...
    try:
        ...
        if is_ekc_no_result_case:
            ...
            try:
                ...
                logging.debug(f"[is_ekc_no_result_case] Исходные значения: fi='{fi}', ii='{ii}', oi='{oi}', nom='{nom}'")

                # Формируем список только непустых частей (исключая строку 'None')
                fio_parts = [part for part in [fi, ii, oi] if part and part != 'None']
                fio_string = ' '.join(fio_parts)

                logging.debug(f"[is_ekc_no_result_case] Сформированная ФИО-строка: '{fio_string}'")

                if fio_string:
                    base_name = f"{fio_string} {nom}-26 нет профиля"
                else:
                    base_name = f"{nom}-26 нет профиля"
                    logging.warning("[is_ekc_no_result_case] Все части ФИО отсутствуют, имя файла будет содержать только номер дела")

                excel_filename = f"{base_name}.xlsm"
                txt_filename = f"{base_name}.txt"

                logging.info(f"[is_ekc_no_result_case] Имя Excel-файла (теоретическое): {excel_filename}")
                logging.info(f"[is_ekc_no_result_case] Имя TXT-файла для создания: {txt_filename}")
        ...
        if is_special_case:
            ...
            try:
                ...
                try:
                    ...
                    # Определяем разделы для копирования
                    sections_to_copy = [
                        ("ОБСТОЯТЕЛЬСТВА ДЕЛА", "ИССЛЕДОВАТЕЛЬСКАЯ ЧАСТЬ", False),
                        ("РЕЗУЛЬТАТЫ", "Государственный судебно-медицинский эксперт", False),
                    ...
                    logging.debug(f"[is_no_result_case] Исходные значения: fi='{fi}', ii='{ii}', oi='{oi}', nom='{nom}'")

                    # Формируем список только непустых частей (исключая строку 'None')
                    fio_parts = [part for part in [fi, ii, oi] if part and part != 'None']
                    fio_string = ' '.join(fio_parts)

                    logging.debug(f"[is_no_result_case] Сформированная ФИО-строка: '{fio_string}'")

                    if fio_string:
                        result_file_name = f"{fio_string} {nom}-26.docx"
                    else:
                        result_file_name = f"{nom}-26.docx"
                        logging.warning("[is_no_result_case] Все части ФИО отсутствуют, имя файла будет содержать только номер дела")

                    logging.info(f"[is_no_result_case] Имя результирующего файла: {result_file_name}")
        ...
        if is_tobolsk_case:
            ...
            try:
                if card_data.get("1") == "СВО_Молов_образец_прямая идентификация":
                    ...
                    logging.info(f"Выбран шаблон Word для 'СВО_Молов_образец_прямая идентификация': {template_name}")
                elif card_data.get("1") == "СВО_Молов БВП_образец_прямая идентификация":
                    template_name = "Для отправки личный"
                    logging.info(f"Выбран шаблон Word для 'СВО_Молов БВП_образец_прямая идентификация': {template_name}")
