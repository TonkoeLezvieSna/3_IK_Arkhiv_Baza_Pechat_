...
def create_and_fill_document(card_data, txt_data, output_folder, template_name):
    try:
        template_folder = Path(card_data["3"])  # Путь к папке с шаблонами
        ...
        if card_data.get("1") in ["Образец СВО Молов фонд", "Образец СВО Молов прямая идентификация", "Образец СВО Ростов"]:
            ...
            tr_value = str(card_data['ТР']).replace('/', '-').replace('\\', '-')
            output_filename = f"{tr_value} {card_data['НОМ']}-25 {ob_last_char}.docx"
        else:
            output_filename = f"{card_data['ФИ']} {card_data['ИИ']} {card_data['ОИ']} {card_data['НОМ']}-25.docx"

        output_path = output_folder / output_filename
        logging.info(f"Путь к выходному файлу: {output_path}")
        try:
            shutil.copy(template_path, output_path)
            logging.info(f"Шаблон скопирован в выходной файл: {output_path}")
        except Exception as e:
...
def main():
    try:
        ...
        # Проверяем, нужно ли показать предупреждение о ФИО в начале
        ...
        if is_special_case:
            ...
            try:
                output_folder = Path(r"U:\ГЕНЕТИКА НОВОЕ ЗДАНИЕ\Ростов СВО\Кости с ИК-2")
                template_name = "Для отправки кость нет результата"
                # Передаем пустой список как txt_data, так как чтение txt файла не требуется
                output_path = create_and_fill_document(card_data, [], output_folder, template_name)
            ...
            try:
                ...
                txt_data = read_and_filter_txt_data(txt_path)  # Читаем и фильтруем данные из файла txt
            except Exception as e:
            ...
            try:
                ...
                convert_txt_to_xml(txt_path, xml_path, object_name_suffix=txt_data[0][0])  # Передаем выбранный объект
            except Exception as e:
                logging.error(f"Ошибка при преобразовании txt в xml: {e}")
                raise
            # Определяем путь для сохранения файлов в зависимости от значения в поле 1
            try:
                if card_data.get("1") == "Образец СВО Молов прямая идентификация":
                    output_folder = Path(r"U:\ГЕНЕТИКА НОВОЕ ЗДАНИЕ\Ростов СВО\Личные профили 2-ДНК (прямая идентификация)")
                    template_name = "Для отправки личный"
                elif card_data.get("1") == "Образец СВО Молов фонд":
                    output_folder = Path(r"U:\ГЕНЕТИКА НОВОЕ ЗДАНИЕ\Ростов СВО\Родственники с 2-ДНК Тюмень")
                    template_name = "Для отправки родственники"
                elif card_data.get("1") == "Кость СВО":
                    output_folder = Path(r"U:\ГЕНЕТИКА НОВОЕ ЗДАНИЕ\Ростов СВО\Кости с ИК-2")
                    template_name = "Для отправки кость"
                elif card_data.get("1") == "Образец СВО Ростов":
                    output_folder = Path(r"U:\ГЕНЕТИКА НОВОЕ ЗДАНИЕ\Ростов СВО\Родственники с 2-ДНК Ростов")
                    template_name = "Для отправки родственники"
                else:
                    output_folder = Path(card_data["6"])
                    template_name = "Для отправки родственники"
            except Exception as e:
                logging.error(f"Ошибка при определении пути для сохранения файлов: {e}")
                raise
            # Создание и заполнение нового файла Word и шаблона
            try:
                output_path = create_and_fill_document(card_data, txt_data, output_folder, template_name)  # Передаем данные из txt
                print(f"Новый документ создан и заполнен: {output_path}")
            except Exception as e:
                logging.error(f"Ошибка при создании и заполнении документа: {e}")
                raise
            ...
            if value_in_column_1 in ["Образец СВО Молов фонд", "Образец СВО Молов прямая идентификация", "Образец СВО Ростов"]:
                ...
                try:
                    output_file = run_vba_macro_2_DNK(vba_path, txt_path, output_folder, card_data)
                    logging.info(f"Файл Excel успешно создан: {output_file}")
                except Exception as e:
                ...
                try:
                    output_file = run_vba_macro_IK_2(vba_path, txt_path, output_folder, card_data)
                    logging.info(f"Файл Excel успешно создан: {output_file}")
                except Exception as e:
                    logging.error(f"Ошибка при выполнении макроса VBA IK-2: {e}")
                    raise
                ...
                try:
                    output_file = run_vba_macro(vba_path, txt_path, output_folder, card_data)
                    logging.info(f"Файл Excel успешно создан: {output_file}")
                except Exception as e:
                    logging.error(f"Ошибка при выполнении макроса VBA: {e}")
                    raise
            ...
            try:
                source_folder = Path(card_data["2"])
                archive_folder = Path(card_data["5"])
                # Формируем путь к документу для печати
                document_to_print = source_folder / f"{card_data['НОМ']}-25.docx"
